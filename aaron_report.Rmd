...
rm(tmpMD)
plt1 <- ggplot(data=adf[adf$EARNINGS_GT_MD == TRUE,], aes(x=C150_4, y=AVGFACSAL), main="More than median") + geom_point(alpha=I(0.08)) 
plt2 <- ggplot(data=adf[adf$EARNINGS_GT_MD == FALSE,], aes(x=C150_4, y=AVGFACSAL), main="Less than median") + geom_point(alpha=I(0.08))
grid.arrange(plt1, plt2, ncol=2)
ggplot(data=adf, aes(x=C150_4, color=EARNINGS_GT_MD, y=AVGFACSAL), ylim=c(0,10^5)) + geom_point(alpha=I(0.08)) + geom_smooth()
```
...
ggplot(data=adf, aes(x=log(FAMINC), y=log(MD_EARN_WNE_P10))) + geom_point(alpha=0.08) + xlim(9.5,12) + ylim(9.5,11.5) + annotate("text", x=11.5, y=10, label = fstat, size=5.5) + annotate("text", x=11.5, y=9.8, label = rsq, size=5.5)
rm(adf)
```